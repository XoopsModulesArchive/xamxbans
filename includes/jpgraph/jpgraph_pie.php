...
// Ver:		$Id: jpgraph_pie.php,v 1.1 2004/08/25 01:12:53 rboyleiii Exp $
//
...
class PiePlot
{
    ...
    public $explode_radius = [];

    public $explode_all = false;
    ...
    public $pie_border = true;

    public $pie_interior_border = true;

    public $value;

    public $ishadowcolor = '';

    public $ishadowdrop = 4;
    ...
    {
        if (array_key_exists($aTheme, $this->themearr)) {
            $this->theme = $aTheme;
        } else {
    ...
    public function SetSize($aSize)
    {
        if (($aSize > 0 && $aSize <= 0.5) || ($aSize > 10 && $aSize < 1000)) {
            ...
            JpGraphError::Raise(
                'PiePlot::SetSize() Radius for pie must either be specified as a fraction
    ...
    public function Legend($graph)
    {
        ...
        if (null === $this->setslicecolors) {
            ...
            if ($graph->pieaa && 'pieplot' === get_class($this)) {
        ...
        for ($i = $n - 1; $i >= 0; --$i) {
            ...
            if (null === $this->setslicecolors) {
                ...
                $graph->legend->Add(
                    $l,
                    $this->setslicecolors[$i % $numcolors],
                    '',
                    0,
                    $this->csimtargets[$i],
                    $alt
...
class PiePlotC extends PiePlot
{
    ...
    public $middlecsimtarget = '';

    public $middlecsimalt = '';
